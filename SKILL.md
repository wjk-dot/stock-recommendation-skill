...
2. 按市场范围与持有周期筛选候选代码，用 a-stock-analysis 采集候选股票当日数据：
   ...
   ```
   打包器和渲染器会拒绝 `???`、`�`、`锟斤拷` 等不可恢复乱码；出现提示时请重新采集 `analysis.json`，不要通过 PowerShell 文本重定向重写中文标注。
   - 加 --json 输出结构化数据
...
5. Codex 调用本 skill 生成 GUI：
   ...
   ```
6. Docker 已启动时，Codex **只打开本次渲染的 VANTA 页面**：`http://127.0.0.1:8765/app/{输出文件名}`；不得同时打开 `templates/workbench.html`、`templates/dashboard.html` 或 `templates/quant-dashboard.html`。未启动 Docker 时才返回 `file://` 链接，并明确说明 K 线、真实回测和资金流需要启动 Docker。
